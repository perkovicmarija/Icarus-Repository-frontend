...
class PdfFrame extends Component {
    ...
    componentDidMount() {
        ...
        PdfJsLib.getDocument(this.props.file).promise.then((pdf) => {
...
/*            this.pdfFindController.onUpdateResultsCount = (matchesCount) => {
    ...
    onPageNumberInputKeyPress = (event) => {
        if (event.key === 'Enter') {
            if(this.isInt(this.state.pageNumberInput) && this.state.pageNumberInput > 0 && this.state.pageNumberInput <= this.state.numPages) {
    ...
    zoomIn(ticks) {
        ...
        this.setState({scale: newScale, customScaleOptionValue: newScale, customScaleOptionTextContent});
    }
    ...
    render() {
        ...
        let toggleSidebarButtonClassName = this.state.sidebarOpen ? 'toolbarButton toggled' : 'toolbarButton';
        let outerContainerClassName = this.state.sidebarOpen ? 'sidebarOpen' : '';
        let toggleFindButtonClassName = this.state.findOpen ? 'toolbarButton toggled' : 'toolbarButton';
        let findBarClassName = this.state.findOpen ? 'findbar doorHanger' : 'findbar hidden doorHanger';
        let findResultsCountClassName = this.state.findResultsCountSnow ? 'toolbarLabel' : 'toolbarLabel hidden';
        let findInputClassName = this.state.findInputFound ? 'toolbarField' : 'toolbarField notFound';
        return (
                    ...
                    <div id="sidebarContainer">
                        <div id="toolbarSidebar">
                            <div className="splitToolbarButton toggled">
                                <button id="viewOutline" className="toolbarButton"
                        ...
                        <div id="sidebarContent">
                            <div id="outlineView" className="outlineWithDeepNesting">
                                {bookmarkItems.map((item) => {
                                    return (
                                        <BookmarkItem key={item.title} item={item} pdfLinkService={this.pdfLinkService} onZoomManualChange={this.onZoomManualChange} />
                                    )
                                })}
                            </div>
                        </div>
                    ...
                    <div id="mainContainer">
                        <div className={findBarClassName} id="findbar">
                            <div id="findbarInputContainer">
                                <input id="findInput" ref={this.inputFind} className={findInputClassName} title="Find" onChange={this.onFindChange} value={queryWord}
                                ...
                                <div className="splitToolbarButton">
                                    <button id="findPrevious" className="toolbarButton findPrevious"
                                            ...
                                            data-l10n-id="find_previous" onClick={this.onFindPreviousClick}>
                                        <span data-l10n-id="find_previous_label">Previous</span>
                                    </button>
                                    <div className="splitToolbarButtonSeparator"></div>
                                    <button id="findNext" className="toolbarButton findNext"
                                            title="Find the next occurrence of the phrase" tabIndex="93"
                                            data-l10n-id="find_next"  onClick={this.onFindNextClick}>
                                        <span data-l10n-id="find_next_label">Next</span>
                                    </button>
                                </div>
                            </div>
                            <div id="findbarOptionsOneContainer">
                                <input type="checkbox" id="findHighlightAll" className="toolbarField" tabIndex="94" name="findHighlightAll" checked={findHighlightAll} onChange={this.onCheckboxChange} />
                                <label htmlFor="findHighlightAll" className="toolbarLabel"
                                       data-l10n-id="find_highlight">Highlight all</label>
                                <input type="checkbox" id="findMatchCase" className="toolbarField" tabIndex="95" name="findMatchCase" checked={findMatchCase} onChange={this.onCheckboxChange}/>
                                <label htmlFor="findMatchCase" className="toolbarLabel"
                                       data-l10n-id="find_match_case_label">Match case</label>
                            </div>
                            <div id="findbarOptionsTwoContainer">
                                <input type="checkbox" id="findEntireWord" className="toolbarField" tabIndex="96" name="findWholeWords" checked={findWholeWords} onChange={this.onCheckboxChange}/>
                                <label htmlFor="findEntireWord" className="toolbarLabel"
                                       data-l10n-id="find_entire_word_label">Whole words</label>
                                <span id="findResultsCount" className={findResultsCountClassName}>{findResultCount}</span>
                            </div>

                            <div id="findbarMessageContainer">
                                <span id="findMsg" className="toolbarLabel">{findMsg}</span>
                            </div>
                        </div>
                        <div id="secondaryToolbar" className="secondaryToolbar hidden doorHangerRight">
                            <div id="secondaryToolbarButtonContainer">

                                <button id="firstPage" className="secondaryToolbarButton firstPage"
                                        title="Go to First Page" tabIndex="56" data-l10n-id="first_page">
                                    <span data-l10n-id="first_page_label">Go to First Page</span>
                                </button>
                                <button id="lastPage" className="secondaryToolbarButton lastPage"
                                ...
                                <button id="pageRotateCw" className="secondaryToolbarButton rotateCw"
                                        title="Rotate Clockwise" tabIndex="58" data-l10n-id="page_rotate_cw">
                                    <span data-l10n-id="page_rotate_cw_label">Rotate Clockwise</span>
                                </button>
                                <button id="pageRotateCcw" className="secondaryToolbarButton rotateCcw"
                            ...
                            <div id="toolbarContainer">
                                <div id="toolbarViewer">
                                    <div id="toolbarViewerLeft">
                                        <button id="sidebarToggle" className={toggleSidebarButtonClassName} title="Toggle Sidebar"
                                                tabIndex="11" data-l10n-id="toggle_sidebar" onClick={this.onSidebarToggle}>
                                            <span data-l10n-id="toggle_sidebar_label">Toggle Sidebar</span>
                                        </button>
                                        <div className="toolbarButtonSpacer" />
                                        <button id="viewFind" className={toggleFindButtonClassName} title="Find in Document"
                                                tabIndex="12" data-l10n-id="findbar" onClick={this.onFindToggle}>
                                        ...
                                        <input onChange={this.onPageNumberInputChange} type="number"
                                               onKeyPress={this.onPageNumberInputKeyPress}
                                               id="pageNumber" className="toolbarField pageNumber"
                                               title="Page" value={pageNumberInput} size="4" min="1" tabIndex="15"
                                               data-l10n-id="page"/>
                                        <span id="numPages" className="toolbarLabel">{numPages}</span>
                                    </div>
                                    <div id="toolbarViewerRight">
                                        {showDownload &&
                                        <button id="download" className="toolbarButton download hiddenMediumView pointer"
                                                title="Download" tabIndex="34" data-l10n-id="download" onClick={this.props.onDownloadClick}>
...
{/*                                        <a href="#" id="viewBookmark" className="toolbarButton bookmark hiddenSmallView" title="Current view (copy or open in new window)" tabIndex="35" data-l10n-id="bookmark">
                                    ...
                                    <div id="toolbarViewerMiddle">
                                        <div className="splitToolbarButton">
                                            <button id="zoomOut" className="toolbarButton zoomOut" title="Zoom Out"
                                                    tabIndex="21" data-l10n-id="zoom_out" onClick={this.onZoomOutClick}>
                                                <span data-l10n-id="zoom_out_label">Zoom Out</span>
                                            </button>
                                            <div className="splitToolbarButtonSeparator"></div>
                                            <button id="zoomIn" className="toolbarButton zoomIn" title="Zoom In"
                                                    tabIndex="22" data-l10n-id="zoom_in" onClick={this.onZoomInClick}>
                                                <span data-l10n-id="zoom_in_label">Zoom In</span>
                                            </button>
                                        </div>
                                        <span id="scaleSelectContainer" className="dropdownToolbarButton">
                                          <select id="scaleSelect" title="Zoom" tabIndex="23" data-l10n-id="zoom"
                                                  onChange={this.onZoomSelectChange} value={scale}>
                                            <option id="pageAutoOption" title="" value="auto" defaultValue="selected"
                                            ...
                                            <option id="pageWidthOption" title="" value="page-width"
                                              ...
                                              <option id="customScaleOption" title="" value={customScaleOptionValue}
                                                      disabled="disabled" hidden={true}>{customScaleOptionTextContent}</option>
