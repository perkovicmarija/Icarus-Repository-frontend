...
const AuditChecklist = (props) => {
  ...
  //Storage
  const checklistDnd = useSelector(
    (state) => state.AuditChecklist.auditDndChecklist
  );
  const checklistItem = useSelector(
    (state) => state.AuditChecklistItem.auditItem
  );
  const checklistSubArea = useSelector(
    (state) => state.AuditChecklistSubArea.auditChecklistSubArea
  );
  const locationTypes = useSelector(
    (state) => state.AuditorActionLocationType.locationTypes
  );
  ...
  const onUpdateOrderClick = () => {
    ...
    selectedChecklistDndCopy.map((dndElement) => {
      const contains = uniqueAffectedParents.find(
        (parentAffected) => parentAffected === dndElement.parent
      );
      if (contains) {
  ...
  const handleInputSubAreaChange = ({ target: { name, value } }) => {
    setSelectedSubArea({
      ...selectedSubArea,
      [name]: value,
    });
  ...
  const handleChecklistItemSave = (item) => {
    ...
    if (viewModel.selectedItem.auditItemId) {
      dispatch(
        auditChecklistItemActions.updateAuditChecklistItemRequest(viewModel)
      );
    } else {
      dispatch(
        auditChecklistItemActions.createAuditChecklistItemRequest(viewModel)
      );
    }
  };
  ...
  const handleDeleteSubArea = () => {
    ...
    dispatch(
      auditChecklistSubAreaActions.deleteAuditChecklistSubAreaRequest(viewModel)
    );
    setDialogDeleteSubareaOpen(false);
    setSubAreaVisible(false);
  };
  ...
  const onSelectTreeItem = (subArea, id) => {
    if (subArea) {
      ...
      dispatch(
        auditChecklistSubAreaActions.getAuditChecklistSubAreaRequest(viewModel)
      );
    } else {
      ...
      dispatch(
        auditChecklistItemActions.getAuditChecklistItemRequest(viewModel)
      );
    }
  ...
  const handleDeleteItem = () => {
    ...
    dispatch(
      auditChecklistItemActions.deleteAuditChecklistItemRequest(viewModel)
    );
    setDialogDeleteItemOpen(false);
    setItemVisible(false);
  };
  ...
  const handleSelectCheckboxChangeItem = (selectedSubAreaId) => {
    ...
    if (selectedItem.auditChecklistSubArea) {
      if (
        selectedItem.auditChecklistSubArea.auditChecklistSubAreaId ===
        selectedSubAreaId
      ) {
        ...
        viewModel.auditChecklistSubArea = {
          auditChecklistSubAreaId: selectedSubAreaId,
        };
        viewModel.auditChecklist = null;
      }
    } else {
  ...
  const handleSelectCheckboxChangeSubArea = (selectedSubAreaId) => {
    ...
    if (selectedSubArea.auditChecklistSubArea) {
      if (
        selectedSubArea.auditChecklistSubArea.auditChecklistSubAreaId ===
        selectedSubAreaId
      ) {
        viewModel.auditChecklistSubArea = null;
        viewModel.auditChecklist = { auditChecklistId: props.match.params.id };
  ...
  const handleDialogSubAreaSubmit = () => {
    ...
    if (viewModel.auditChecklistSubArea.auditChecklistSubAreaId === null) {
      dispatch(
        auditChecklistSubAreaActions.createAuditChecklistSubAreaRequest(
          viewModel
        )
      );
    } else {
      dispatch(
        auditChecklistSubAreaActions.updateAuditChecklistSubAreaRequest(
          viewModel
        )
      );
    }
  ...
  const affectedParentsSetter = (checklistDnd) => {
    ...
    const changedDndElement = checklistDnd.find(
      (dndElement) => dndElement.id === selectedDndElementCopy.id
    );

    const affectedParentsCopy = cloneDeep(affectedParents);

    if (selectedDndElementCopy.parent !== changedDndElement.parent) {
      affectedParentsCopy.push(
        selectedDndElementCopy.parent,
        changedDndElement.parent
      );
    } else if (
      selectedDndElementCopy.data.sequence !== changedDndElement.data.sequence
    ) {
    ...
    newChecklistDnd.forEach((dndElement) => {
      if (dndElement.parent === props.match.params.id) {
        ...
        if (dndElement.data.auditChecklistSubArea) {
          dndElement.data.auditChecklistSubArea.auditChecklistSubAreaId =
            dndElement.parent;
        } else {
  ...
  const setDndElementSequence = (newChecklistDnd) => {
    ...
    for (let i = 0; i < newChecklistDnd.length; i++) {
      ...
      const seqResult = seqDb.find(
        (element) => element.parent === dndElement.parent
      );
      if (!seqResult) {
        seqDb.push({
  ...
  const handleSelectLocationType = (index) => (event) => {
    ...
    selectedItemCopy.auditorActions[index].locationType = locationTypes.find(
      (locationType) => {
  ...
  const handleDeleteAuditorAction = (index) => {
    const selectedItemCopy = cloneDeep(selectedItem);
    selectedItemCopy.auditorActions = selectedItemCopy.auditorActions.filter(
      (auditorAction, aaIndex) => {
            ...
            <div>
              {itemVisible ? (
                <ChecklistItemCreateForm
                  ...
                  checklistType={
                    checklistDnd.auditChecklist.auditChecklistType
                      ? checklistDnd.auditChecklist.auditChecklistType.name
                      : ""
                  }
              ...
              {itemVisible === false && subAreaVisible === false ? (
                ...
                >
                  <label>
                    <IntlMessages id="qms.checklist.noItemSelected" />
                  </label>
                </div>
              ) : null}
            </div>
