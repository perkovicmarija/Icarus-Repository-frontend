...
export function* icarusDocumentationFilesRequest() {
  yield takeLatest(
    types.LOAD_ICARUS_DOCUMENTATION_FILES_REQUEST,
    function* (action) {
      try {
        const response = yield call(
          IcarusDocumentationFileApi.getAll,
          action.viewModel
        );
        debugger;
...
export function* icarusDocumentationFileRequest() {
  yield takeLatest(
    types.LOAD_ICARUS_DOCUMENTATION_FILE_REQUEST,
    function* (action) {
      try {
        const response = yield call(
          IcarusDocumentationFileApi.get,
          action.viewModel
        );
        if (response.code === "200") {
          const data = response.data;
...
export function* createDownloadExcelListRequest() {
  yield takeLatest(
    types.CREATE_ICARUS_DOCUMENTATION_FILE_DOWNLOAD_LIST_EXCEL_REQUEST,
    function* (action) {
      try {
        const channel = yield call(
          IcarusDocumentationFileApi.createDownloadExcelList,
          action.viewModel
        );
        try {
          while (true) {
...
export function* moveRequest() {
  yield takeLatest(
    types.ICARUS_DOCUMENTATION_FILE_MOVE_REQUEST,
    function* (action) {
      try {
        const response = yield call(
          IcarusDocumentationFileApi.moveFile,
          action.viewModel
        );
...
export function* loadInFolderRequest() {
  yield takeLatest(
    types.LOAD_ICARUS_DOCUMENTATION_FILES_IN_FOLDER_REQUEST,
    function* (action) {
      try {
        const response = yield call(
          IcarusDocumentationFileApi.getAllInFolder,
          action.viewModel
        );
        if (response.code === "200") {
          const data = response.data;
          ...
          yield put({
            type: types.LOAD_ICARUS_DOCUMENTATION_FILES_IN_FOLDER_FAILED,
          });
          yield put({
            type: types.AJAX_FAILED,
...
export function* loadBySearchRequest() {
  yield takeLatest(
    types.LOAD_ICARUS_DOCUMENTATION_FILES_BY_SEARCH_REQUEST,
    function* (action) {
      try {
        const response = yield call(
          IcarusDocumentationFileApi.getBySearch,
          action.viewModel
        );
        if (response.code === "200") {
          const data = response.data;
...
export function* loadByFilterRequest() {
  yield takeLatest(
    types.LOAD_ICARUS_DOCUMENTATION_FILES_BY_FILTER_REQUEST,
    function* (action) {
      try {
        const response = yield call(
          IcarusDocumentationFileApi.getByFilter,
          action.viewModel
        );
        if (response.code === "200") {
          const data = response.data;
...
export function* createRequest() {
  yield takeLatest(
    types.CREATE_ICARUS_DOCUMENTATION_FILE_REQUEST,
    function* (action) {
      try {
        const response = yield call(
          IcarusDocumentationFileApi.create,
          action.viewModel
        );
        if (response.code === "200") {
          const data = response.data;
...
export function* uploadRequest() {
  yield takeLatest(types.UPLOAD_ICARUS_DOCUMENTATION_FILE, function* (action) {
    try {
      const channel = yield call(
        IcarusDocumentationFileApi.uploadFile,
        action.viewModel
      );
      try {
        while (true) {
...
export function* editRequest() {
  yield takeLatest(
    types.EDIT_ICARUS_DOCUMENTATION_FILE_REQUEST,
    function* (action) {
      try {
        const response = yield call(
          IcarusDocumentationFileApi.edit,
          action.viewModel
        );
        if (response.code === "200") {
          yield put({
...
export function* downloadFileByGetRequest() {
  downloadTask = yield takeLatest(
    types.DOWNLOAD_ICARUS_DOCUMENTATION_FILE,
    function* (action) {
      let channel;
      try {
        const source = CancelToken.source();
        channel = yield call(
          IcarusDocumentationFileApi.download,
          action.viewModel
        );
        try {
          while (true) {
...
export function* cancelDownloadFile() {
  yield takeLatest(
    types.CANCEL_DOWNLOAD_ICARUS_DOCUMENTATION_FILE,
    function* (action) {
      if (downloadTask) {
...
export function* viewFileRequest() {
  viewTask = yield takeLatest(
    types.VIEW_ICARUS_DOCUMENTATION_FILE,
    function* (action) {
      try {
        const channel = yield call(
          IcarusDocumentationFileApi.view,
          action.viewModel
        );
...
export function* cancelViewFile() {
  yield takeLatest(
    types.CANCEL_VIEW_ICARUS_DOCUMENTATION_FILE,
    function* (action) {
      if (viewTask) {
...
export function* deleteRequest() {
  yield takeLatest(
    types.DELETE_ICARUS_DOCUMENTATION_FILE_REQUEST,
    function* (action) {
      try {
        const response = yield call(
          IcarusDocumentationFileApi.delete,
          action.viewModel
        );
        if (response.code === "200") {
          const data = response.data;
